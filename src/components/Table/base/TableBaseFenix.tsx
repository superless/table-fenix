    ...
    return (
      <>
        <Table compact celled selectable color='violet'>
          <Table.Header>
            <Table.Row>
              {suggestHeaders.map(h => (<TableHeader {...h} related={Related.SUGGESTION} />))}
              {strHeaders.map(h => (<TableHeader {...h} related={Related.STR} />))}
              {num32Headers.map(h => (<TableHeader {...h} related={Related.NUM32} />))}
              {num64Headers.map(h => (<TableHeader {...h} related={Related.NUM64} />))}
              {doubleHeaders.map(h => (<TableHeader {...h} related={Related.DBL} />))}
              {boolHeaders.map(h => (<TableHeader {...h} related={Related.BOOL} />))}
            ...
            {entities!.map(entity =>
              <Table.Row key={entity.id}>
                {suggestHeaders.map((h, i) => (
                  <TableCell related={Related.SUGGESTION} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {strHeaders.map((h, i) => (
                  <TableCell related={Related.STR} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {num32Headers.map((h, i) => (
                  <TableCell related={Related.NUM32} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {num64Headers.map((h, i) => (
                  <TableCell related={Related.NUM64} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {doubleHeaders.map((h, i) => (
                  <TableCell related={Related.DBL} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {boolHeaders.map((h, i) => (
                  <TableCell related={Related.BOOL} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {enumHeaders.map((h, i) => (
                  <TableCell related={Related.ENUM} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {dtHeaders.map((h, i) => (
                  <TableCell related={Related.DATE} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {/* {geoHeaders.map(h => (
                ...
                {RelatedHeaders.map((h, i) => (
                  <TableCell related={Related.REFERENCE} entity={entity} typeSearchIndex={h.index} key={`${entity.id}_${i}`} />
                ))}
                {this.props.cells && this.props.cells.map(ch => (
                  <Table.Cell textAlign='center' >{ch(entity)}</Table.Cell>
                ))}
              </Table.Row>
            )}
          </Table.Body>
