...
import { Table, Pagination, PaginationItemProps, Segment, Button } from 'semantic-ui-react';
...
export default function TableFenix(props: ITableFenixProps) {
  ...
  const enumHeaders = GetPropHeaders(entities, Related.ENUM, props.headerProperty);
  ...
  const setPaginationValue: (event: React.MouseEvent<HTMLAnchorElement>, data: PaginationItemProps) => void = async (e, d) => {
    props.selectPage &&  props.selectPage(d.activePage);
  ...
  return (
    <>
      <Table compact celled selectable color='violet'>
        <Table.Header>
          <Table.Row>
            ...
            {props.cellheaders && props.cellheaders.map(ch=>(
        ...
        <Table.Body>
          ...
          {entities!.map(entity =>
            <Table.Row key={entity.id}>
              ...
              {enumHeaders.map(h => (
                <Table.Cell textAlign='center' key={`${Related.ENUM as number}.${h.index}_${entity.id}`}> {entity.enum.filter(e => e.propertyIndex === h.index).length > 0 ? props.enumValue(h.index, entity.enum.filter(e => e.propertyIndex === h.index)[0].value) : ""}</Table.Cell>
              ))}
              {dtHeaders.map(h => (
                <Table.Cell textAlign='center' key={`${Related.DATE as number}.${h.index}_${entity.id}`}> {entity.dt.filter(e => e.propertyIndex === h.index).length > 0 ? moment(entity.dt.filter(e => e.propertyIndex === h.index)[0].value).format("DD-MM-yyyy HH:mm") : ""}</Table.Cell>
              ))}
              {/* {geoHeaders.map(h => (
              ...
              {props.cells && props.cells.map(ch=>(
      ...
      <Segment textAlign='center' basic>
          <Pagination
                  stackable
                  disabled={ !props.elements || props.elements.total === 0}
                  activePage={props.elements?.current??1}
                  onPageChange={setPaginationValue}
                  totalPages={props.elements && Math.ceil(props.elements.total / props.itemPerPage)|| 1}
                />
              </Segment>
    </>
  );
